...
exports.getCommerceMemoryContext = getCommerceMemoryContext;
exports.hasRelevantIntent = hasRelevantIntent;
const rez_intent_graph_1 = require("rez-intent-graph");
const logger_1 = require("../logger");
// ── Get Commerce Memory Context ────────────────────────────────────────────────
async function getCommerceMemoryContext(userId, options = {}) {
    const { includeActive = true, includeDormant = true, includeProfile = true } = options;
    try {
        logger_1.logger.info('[CommerceMemoryContext] Loading context', { userId });
        // Get enriched context from Intent Graph
        const enrichedContext = await rez_intent_graph_1.crossAppAggregationService.getEnrichedContext(userId);
        if (!enrichedContext) {
            logger_1.logger.info('[CommerceMemoryContext] No context found', { userId });
            return null;
        }
        ...
        const activeIntents = filteredActive.map((intent) => ({
            key: intent.key,
        ...
        logger_1.logger.info('[CommerceMemoryContext] Context loaded', {
            userId,
        ...
        logger_1.logger.error('[CommerceMemoryContext] Failed to load context', { userId, error });
        return null;
    }
}
// ── Quick Context Check ────────────────────────────────────────────────────────
async function hasRelevantIntent(userId, query) {
    ...
    for (const intent of context.activeIntents) {
        if (intent.key.toLowerCase().includes(queryLower) ||
...
function formatContextForAI(activeIntents, dormantIntents, profile) {
    const parts = [];
    ...
    if (activeIntents.length > 0) {
        ...
        for (const intent of activeIntents.slice(0, 5)) {
            parts.push(`- ${intent.displayName} (${Math.round(intent.confidence * 100)}% interest, ${intent.lastSeen})`);
        }
    }
    ...
    if (dormantIntents.length > 0) {
        ...
        for (const intent of dormantIntents.slice(0, 3)) {
            parts.push(`- ${intent.displayName} (${intent.daysDormant} days ago, ${Math.round(intent.revivalScore * 100)}% likely to convert)`);
        }
    }
    // Profile
    parts.push('\n### User Profile:');
...
function formatIntentDisplayName(key, category) {
    // Convert "hotel_goa_weekend" → "Hotels in Goa for weekend"
    const parts = key
        .split('_')
        .filter((w) => !['view', 'search', 'hold', 'hotel', 'restaurant', 'retail', 'order', 'cart'].includes(w.toLowerCase()));
    if (parts.length === 0) {
...
function getActionSuggestion(intent) {
...
function getPreferredChannel(profile) {
    const max = Math.max(profile.travelAffinity, profile.diningAffinity, profile.retailAffinity);
    if (max === profile.travelAffinity)
        return 'email';
    if (max === profile.diningAffinity)
        return 'push';
    return 'in_app';
}
function getDominantCategory(profile) {
    const max = Math.max(profile.travelAffinity, profile.diningAffinity, profile.retailAffinity);
    if (max === profile.travelAffinity)
        return 'Travel';
    if (max === profile.diningAffinity)
...
function generateRecommendations(active, dormant, profile) {
    const recs = [];
...
//# sourceMappingURL=commerceMemoryContext.js.map