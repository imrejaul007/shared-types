...
const HOTEL_APP_TYPE = 'hotel_ota';
...
export async function captureBookingConfirmed(params) {
    ...
    if (result.intent.status === 'FULFILLED') {
        ...
        const revived = dormantIntents.find((di) => di.intentKey.includes(params.hotelId || ''));
        if (revived) {
 ...
 */
function buildHotelIntentKey(action, params) {
    const parts = [
        'hotel',
        action,
 ...
 */
export async function getActiveTravelIntents(userId) {
...
//# sourceMappingURL=hotelOtaIntegration.js.map