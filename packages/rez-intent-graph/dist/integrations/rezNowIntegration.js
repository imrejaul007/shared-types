...
const RESTAURANT_APP_TYPE = 'restaurant';
const DINING_CATEGORY = 'DINING';
/**
 * Capture restaurant/merchant search intent
 */
export async function captureRestaurantSearch(params) {
 ...
 */
export async function captureMerchantView(params) {
 ...
 */
export async function captureAddToCart(params) {
 ...
 */
export async function captureCheckoutStart(params) {
 ...
 */
export async function captureOrderPlaced(params) {
    ...
    if (result.intent.status === 'FULFILLED') {
        ...
        const revived = dormantIntents.find((di) => di.intentKey.includes(params.merchantId || ''));
        if (revived) {
 ...
 */
export async function captureCartAbandoned(params) {
 ...
 */
function buildRestaurantIntentKey(action, params) {
    const parts = [
        'restaurant',
        action,
 ...
 */
export async function getActiveDiningIntents(userId) {
...
//# sourceMappingURL=rezNowIntegration.js.map