...
const logger = {
    ...
    error: (msg, meta) => console.error(`[RevenueAgent] ${msg}`, meta || ''),
};
// ── Agent Configuration ────────────────────────────────────────────────────────
export const revenueAttributionAgentConfig = {
...
export async function generateRevenueReport(startDate, endDate) {
    ...
    const [revenue, roiByChannel, roiByMerchant, conversionLift, topNudges, underperforming] = await Promise.all([
...
export async function handleConversionAttribution(record) {
...
async function checkForRevenueAlerts(report) {
...
async function executeRevenueActions(report) {
...
// ── Main execution ─────────────────────────────────────────────────────────────
export async function runRevenueAttributionAgent() {
...
export function startRevenueAttributionCron() {
    if (cronInterval)
        return;
    ...
    cronInterval = setInterval(() => runRevenueAttributionAgent().catch((err) => logger.error('Revenue cron failed', { error: err })), revenueAttributionAgentConfig.intervalMs);
}
export function stopRevenueAttributionCron() {
    if (cronInterval) {
        clearInterval(cronInterval);
        cronInterval = null;
    }
}
//# sourceMappingURL=revenue-attribution-agent.js.map