...
const logger = {
    info: (msg, meta) => console.log(`[NetworkEffectAgent] ${msg}`, meta || ''),
    warn: (msg, meta) => console.warn(`[NetworkEffectAgent] ${msg}`, meta || ''),
    error: (msg, meta) => console.error(`[NetworkEffectAgent] ${msg}`, meta || ''),
};
// ── Agent Configuration ────────────────────────────────────────────────────────
export const networkEffectAgentConfig = {
...
// ── Build user features ────────────────────────────────────────────────────────
async function buildUserFeatures(userId) {
...
function cosineSimilarity(a, b) {
    const dotProduct = a.travelAffinity * b.travelAffinity +
...
async function clusterUsers() {
    ...
    const users = userResults.map((r) => r._id);
    if (users.length < 10) {
...
// ── Find similar users ─────────────────────────────────────────────────────────
export async function findSimilarUsers(userId, limit = 10) {
    ...
    const otherUsers = userResults.map((r) => r._id);
    const similarities = [];
...
export async function getCollaborativeRecommendations(userId, category, limit = 5) {
...
export async function generateCollaborativeSignal(userId) {
...
async function triggerCohortNudge(signal) {
    // Only nudge if strong collaborative signal
    if (signal.collaborativeFilterScore < 0.5)
        return;
...
export async function triggerCohortCampaign(category) {
    const clusterKey = `${category}_high_value`;
    const cluster = await sharedMemory.get(`cluster:${clusterKey}`);
...
async function analyzeTrendingSignals() {
...
// ── Main execution ─────────────────────────────────────────────────────────────
export async function runNetworkEffectAgent() {
    ...
    try {
        ...
        const activeUsers = activeUsersResult.map((r) => r._id);
...
export async function getCollaborativeSignalForUser(userId) {
...
export function startNetworkEffectCron() {
    if (cronInterval)
        return;
    ...
    cronInterval = setInterval(() => runNetworkEffectAgent().catch((err) => logger.error('Network effect cron failed', { error: err })), networkEffectAgentConfig.intervalMs);
}
export function stopNetworkEffectCron() {
    if (cronInterval) {
        clearInterval(cronInterval);
        cronInterval = null;
    }
}
//# sourceMappingURL=network-effect-agent.js.map