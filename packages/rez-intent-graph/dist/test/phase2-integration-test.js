 ...
 */
import { chargeWallet, creditWallet, getWalletBalance, createOrder, updateOrderStatus, executeRoomServiceFlow, executeShoppingFlow, getCircuitBreakerStatus, resetCircuitBreaker, getAllServiceHealth, } from '../integrations/external-services.js';
...
async function testWalletService() {
    ...
    const creditResult = await creditWallet('test-user-001', 100, 'Test credit from ReZ Mind', { referenceType: 'bonus' });
    console.log('   Credit result:', creditResult.success ? '✅ success' : '⚠️ failed (expected)');
    // Test charge
    console.log('🧪 Testing: Charge wallet');
    const chargeResult = await chargeWallet('test-user-001', 50, 'Test charge from ReZ Mind', { referenceType: 'order' });
...
async function testRoomServiceFlow() {
    ...
    const result = await executeRoomServiceFlow('guest-001', '101', 'hotel-001', [
...
async function testShoppingFlow() {
    ...
    const result = await executeShoppingFlow('user-001', 'store-001', 'merchant-001', [
...
main();
//# sourceMappingURL=phase2-integration-test.js.map