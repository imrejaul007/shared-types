...
export class CrossAppAggregationService {
   ...
   */
  async getProfile(userId: string): Promise<CrossAppIntentProfile> {
    return (await prisma.crossAppIntentProfile.upsert({
      where: { userId },
      create: { userId },
      update: {},
    })) as unknown as CrossAppIntentProfile;
  }
  ...
  async getEnrichedContext(userId: string): Promise<EnrichedContext> {
    ...
    const activeList = activeIntents.map((i: { category: string; intentKey: string; confidence: unknown; lastSeenAt: Date }) => ({
    ...
    const dormantList = dormantIntents.map((di: { category: string; intentKey: string; revivalScore: unknown; daysDormant: number }) => ({
      category: di.category as Category,
      key: di.intentKey,
  ...
  async aggregateMerchantDemand(
    ...
    const avgConfidence =
      totalCount > 0
        ? intents.reduce((sum: number, i: { confidence: unknown }) => sum + Number(i.confidence), 0) / totalCount
        : 0;

    // Group by intent key
    const intentCounts = intents.reduce((acc: Record<string, number>, i: { intentKey: string }) => {
      acc[i.intentKey] = (acc[i.intentKey] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const topIntentKeys = Object.entries(intentCounts as Record<string, number>)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      .map(([key]) => key);
   ...
   */
  private calculateTrend(count: number, category: string): 'rising' | 'stable' | 'declining' {
  ...
  async getUserAffinities(userId: string): Promise<{
    ...
    const categories: Array<{ category: Category; score: number }> = [
