...
import {
  type ScoringContext,
  type DormancyDetection,
  type RevivalCandidate,
  BASE_CONFIDENCE,
  DORMANCY_THRESHOLD_DAYS,
  CONFIDENCE_DORMANT_THRESHOLD,
...
export class IntentScoringService {
  ...
  async detectDormantIntents(daysThreshold = DORMANCY_THRESHOLD_DAYS): Promise<DormancyDetection[]> {
    ...
    return activeIntents.map((intent: { id: string; lastSeenAt: Date; confidence: unknown }) => ({
      intentId: intent.id,
      ...
      shouldMarkDormant:
        Number(intent.confidence) < CONFIDENCE_DORMANT_THRESHOLD ||
  ...
  async getRevivalCandidates(
    ...
    return dormantIntents.map((di: {
      ...
      dormantIntent: di as unknown as RevivalCandidate['dormantIntent'],
      intent: di.intent as unknown as RevivalCandidate['intent'],
